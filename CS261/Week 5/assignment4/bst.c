...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	int compval = compare(val, cur->val);

	if (compval == -1) cur->left = _addNode(cur->left, val);
	else if (compval == 1) cur->right = _addNode(cur->right,val);
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	temp = tree->root;
	while(temp->val != 0){
		int compval = compare(val, temp->val);
		if(compval == 0) return 1;
		else if (compval == -1) temp = temp->left;
		else if (compval == 1) temp = temp->right;
		if (temp == 0) break;
	}
	free(temp);
	return 0;
