...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
	if(compare(getDynArr(heap, i), getDynArr(heap, j)) == 1) return i;
	return j;
}
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	assert (last != 0);
    addDynArr(heap, val);
    _adjustHeap(heap,last,0);//_adjustHeap(heap,last,sizeDynArr(heap));
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   ...
   int rghtChild = 2*pos +2;
   int	smallChildPos; //get index of smallest child
   if (rghtChild < max){//we have two children
	   //printf("rightchild \n");//debug
	   if(compare(getDynArr(heap,rghtChild), getDynArr(heap,leftChild)) == 1) //right is bigger
		   smallChildPos = leftChild;
	   else smallChildPos = rghtChild; //left is bigger

	   if (compare(getDynArr(heap,pos), getDynArr(heap,smallChildPos)) == 1){
		   swapDynArr(heap, pos, smallChildPos);
	   }
	   _adjustHeap(heap, max, smallChildPos);
...
void _buildHeap(DynArr *heap)
{
	...
	assert (max > 0);
	int index = (max/2)+1; //last non-leaf node
	while(index>=0){
...
void sortHeap(DynArr *heap)
{
	int first = 0;
	int last = sizeDynArr(heap)-1;
	//printf("Sort Called"); //debug
	_buildHeap(heap);
	while(last > first){
		swapDynArr(heap, first, last);
		_adjustHeap(heap, --last, first);
	}
