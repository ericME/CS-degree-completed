...
int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed,
  ...
  //check selected kingdom cards are different and 
  for (i = 0; i < 10; i++)
    {
      for (j = 0; j < 10; j++)
        {
	  if ((j != i && kingdomCards[j] == kingdomCards[i]) || (kingdomCards[j] < curse || kingdomCards[j] > treasure_map) )
	    {
	      return -1;
	    }
        }
...
int zzzminecardplay(int i, int j, int currentPlayer, int handPos, int choice1, 
  int choice2, struct gameState * state){
...
void zzzbaroncardplay(int currentPlayer, int choice1, struct gameState* state) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
      return zzzminecardplay(i, j, currentPlayer, handPos, choice1, choice2, state);
